...
test("loads fixture leaderboard and reveals more on click", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { name: /Hygienica Leaderboard/i })).toBeVisible();
  await expect(page.getByTestId("leader-row-1")).toBeVisible();
  await expect(page.getByText(/Nancy Meke/)).toBeVisible();
